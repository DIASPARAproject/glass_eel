...
author:
  - Tessa van der Hammen:
      email: tessa.vanderhammen@wur.nl
...
institute:
  - wageningen: Wageningen Marine Research, PO Box 68, 1970 AB IJmuiden, Netherlands
...
*Anguilla*, glass eel, size

\newpage

# Introduction

-   Species in the Anguilla genus, such as the European eel (Anguilla
    anguilla) and the American Eel (Anguilla Rostrata) are facultatively
    catadromous fish.

Because the smallest eel larvae were caught in the vicinity of the
Sargasso Sea, spawning is assumed to occur there (Schmidt 1923, Schoth &
Tesch 1982). As leptocephali larvea eel cross the Atlantic Ocean. When
they reach the European continental shelf the leptocephali metamorphose
into glass eels, where most try to migrate upstream on the European and
North African rivers. After they reached the European coaste, glass eel
start to pigment and become yellow eels.

-successful oceanographic migration depends on oceanographic conditions
which determine leptocephali transport, feeding and mortality.

-glasseels arrive at the inland waters at different sizes per location,
but also there is variation within location

\- Larvae using shorter routes produce smaller glasseels.

-Lecomte-Finiger 1992 is wrong? not every dayring is counted? There is a
table with estimated of the migration time.
...
The glass eels sampled from the Northern Ireland coastline sites of
...
Glass eel monitoring at Den Oever was carried out since 1938 with a 1m2
liftnet at night at two-hourly intervals during spring. The sampling
location is right in front of the discharge sluices in the dike that
separates a large freshwater lake (Lake IJsselmeer, 1820 km2) from the
brackish Wadden Sea. Since 1960 glass eel lengths have been measured,
with gaps in 2002, 2004-2011 and 2014. In principle the measurements
were taken from the glass eels caught in the liftnet monitoring. When
that resulted in low numbers, in some years additional sampling took
place at the nearby ship locks. Sampling occurred at night, and the
glass eels were anaesthetized in a solution of MS222 and measured to the
nearest millimeter. In most of the years, length measurements were taken
once a week during March-April. The main glass eel season in the
Netherlands is from March to June, with the peak in April. Because of
the decline in glass eel arrival in the 1980's, the number of glass eels
measured is lower in the more recent years because the number of glass
eels caught were not always sufficient for larger samples.

Scotland - Shielddaig

East River Canada - *Anguilla rostrata*
...
don$season<-paste0(year(don$fi_date)-1,"-",year(don$fi_date)) # year-1-year. How does this work;it adds the year before, but doesn't start in november?
...
```{r make info_table, echo=FALSE}

# table with location name, start year, end year, length presence/absence 

df <- don %>%
   group_by(ser_nameshort) %>%  
   summarise(min_year= min(fi_year),
             max_year= max(fi_year),
             total_nr_years = n_distinct(fi_year),
             length_measurements =  sum(!is.na(lengthmm)),
             weight_measurements =  sum(!is.na(weightg)))
df

# this doesn't work in word
#df %>%
 # kbl() %>%
  #kable_styling()
...
```{r exploratory_plots, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
knitr::opts_chunk$set(echo=FALSE)
...
ggplot(don)+ geom_boxplot(aes(x=fi_year, y=(lengthmm)))+
...
# make lf distribution
ggplot(don[don$lengthmm <95 & don$lengthmm>45,], aes(x=lengthmm, color=ser_nameshort)) +  geom_histogram(binwidth=1)
...
don1 <- don[, c("lengthmm", "ser_nameshort", "time", 'yday', 'fi_date', 'fi_year')]
don1 <- don1[don1$fi_year >1990,]
don1 <- don1[complete.cases(don1),]
#don1 <- don1[don1$ser_nameshort %in% c('RhDOG', 'Oria','VilTG','BannG','ShiMG'),]
...
# get back to dates
v2$fit$time=as.Date(v2$fit$time, origin=origin)
v2$res$time=as.Date(v2$res$time, origin=origin)
