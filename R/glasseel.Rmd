...
OR   <- read.csv(file.path(datawd, "ES_Oria.csv", sep=""), sep=";")
  ...
  # Bages-Sigean, Mediteranean SPAIN, 1 location
BS <- read.csv(file.path(datawd, "ES_BagesSigean.csv", sep=""), sep=";")
  BS$fi_date2 <- as.Date(BS$fi_date, "%d-%m-%Y")
  any(is.na(BS$fi_date2))
  BS$fi_year <- format(as.Date(BS$fi_date2), "%Y")
  BS$fi_comment <- NA
  BS$fi_id_cou <- NA
...
EA2024   <- read.csv(file.path(datawd, "CA_Eastriver2024.csv", sep=""), sep=";")
EA2025   <- read.csv(file.path(datawd, "CA_Eastriver2025.csv", sep=""), sep=";")
 ...
 EA2024$fi_date2 <- as.Date(paste(as.character(EA2024$year),"-", EA2024$month,"-", EA2024$day, sep='')) 
 EA2024$fi_date2 <- as.Date(EA2024$fi_date2,"%d-%m-%Y")
 
 EA2025$fi_date2 <- as.Date(EA2025$date,"%d-%m-%Y")
 EA2025$year     <- 2025
 
 EA24_25 <- rbind(EA2024[,c('fi_date2', 'length', 'weight', 'year', 'stage')],
                  EA2025[,c('fi_date2', 'length', 'weight', 'year', 'stage')])
  
 EA24_25$fi_year  <- EA24_25$year
 EA24_25$lengthmm <- EA24_25$length
 EA24_25$weightg <- EA24_25$weight
 EA24_25$ser_nameshort <- 'East_River'
 EA24_25$fi_comment <- ''
 EA24_25$ge_stage <- EA24_25$stage 
 EA24_25$fi_id_cou <- ''
...
dat <- rbind(DO[,c("fi_id_cou","ser_nameshort","fi_date2","fi_year","ge_stage","lengthmm","weightg","fi_comment")],
             ...
             BS[,c("fi_id_cou","ser_nameshort","fi_date2","fi_year","ge_stage","lengthmm","weightg","fi_comment")],
             EA[,c("fi_id_cou","ser_nameshort","fi_date2","fi_year","ge_stage","lengthmm","weightg","fi_comment")],
             EA24_25[,c("fi_id_cou","ser_nameshort","fi_date2","fi_year","ge_stage","lengthmm","weightg","fi_comment")],
...
dat$weightg <- as.numeric(dat$weightg)

table(dat$ser_nameshort, dat$fi_year)
...
#  geom_point(data = df[df$ser_nameshort %in% c( "RhDOG","VilTG","VilG", "ShiMG", "BannG", "StraG" , "ErneG", "Oria","Burrishoole", 'BagesSigean' ),],
#             aes(x = ser_x, y = ser_y, color='red'), size = 1) + 
scale_fill_grey() +theme_map()+
  geom_label(data = df[df$ser_nameshort %in% c( "RhDOG","VilTG","VilG", "ShiMG", "BannG", "StraG" , "ErneG", "Oria","Burrishoole", 'BagesSigean' ),],
...
ggplot(dat[dat$ser_nameshort %in% 'East_River' & dat$lengthmm >40 & dat$lengthmm <80,])+
  geom_point(aes(x=fi_date2, y=lengthmm)) +
